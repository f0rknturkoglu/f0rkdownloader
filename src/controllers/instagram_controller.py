...
Handles all Instagram-related UI operations.
"""
...
class InstagramController(BaseController):
    """Controller for Instagram operations."""

    def __init__(self, config: Any, ui: Any):
        super().__init__(config, ui)
        self.downloader = InstagramDownloader(config)

    def run(self) -> None:
        """Main Instagram menu loop."""
        self.ui.push_breadcrumb("Instagram")
        ...
        while True:
            ...
            try:
                ...
                self.handle_error(e, "Instagram operation")
    ...
    def handle_single(self) -> None:
        ...
        import questionary
        url = questionary.text("Instagram URL (Reel / Gönderi):", style=self.ui.custom_style).ask()
        if not url:
            return

        self.ui.console.print(f"\n[bold green]İndiriliyor...[/bold green] [dim]{url}[/dim]\n")
        self.logger.download_start("instagram", url)

        success, message = self.downloader.download(url)

        if success:
            self.ui.show_success(message)
            self.logger.download_success("instagram", url)
        else:
            self.ui.show_error(message)
            self.logger.download_fail("instagram", url, message)
    ...
    def handle_auto_scan(self) -> None:
        """Automatically detect the latest Instagram URL list in Downloads folder."""
        ...
        candidates = [
            ...
            and ("instagram" in f.name.lower() or "f0rkn" in f.name.lower() or "url" in f.name.lower())
        ]

        if not candidates:
            # Fallback to any recent txt file in Downloads
