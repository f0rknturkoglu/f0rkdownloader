...
Uses gallery-dl and yt-dlp dual-engine for Pinterest Pins, Videos, and Boards.
Supports single and bulk downloads.
"""
...
class PinterestDownloader(DownloaderBase):
    """Pinterest downloader using yt-dlp and gallery-dl dual-engine."""

    SUPPORTED_DOMAINS = ("pinterest.", "pin.it")

    def __init__(self, config: Any):
        super().__init__(config)
        pin_path = getattr(config, "pinterest_path", Path(config.download_path) / "Pinterest")
        self.pinterest_download_path = Path(pin_path)
        self.pinterest_download_path.mkdir(parents=True, exist_ok=True)
        self.history = DownloadHistory(
            config.download_path,
            platform_paths={"pinterest": self.pinterest_download_path}
        )

    def is_pinterest_url(self, url: str) -> bool:
        """Check if URL is a valid Pinterest URL."""
        return any(domain in url for domain in self.SUPPORTED_DOMAINS)

    @classmethod
    def normalize_url(cls, url: str) -> str:
        """
        Normalize Pinterest URLs.
        """
        if not url:
            return url
        url = url.strip()
    ...
    def _download_with_gallery_dl(self, url: str) -> tuple[bool, str]:
        """
        Downloader for Pinterest using gallery-dl (super strong on Pinterest).
        """
        ...
        cmd = [
            gallery_dl_path,
            "--directory", str(self.pinterest_download_path),
            "--filename", "{id}_{title[:100]}.{extension}",
            url
        ]
        if self.config.pinterest_cookies_file and os.path.exists(self.config.pinterest_cookies_file):
            cmd.extend(["--cookies", self.config.pinterest_cookies_file])
    ...
    def read_urls_from_file(self, file_path: str) -> list[str]:
        """
        Read Pinterest URLs from a text file or folder.
        """
        path = Path(file_path)
        if not path.exists():
        ...
        try:
            with open(path, "r", encoding="utf-8") as f:
                for line in f:
                    url = line.strip()
                    if url and self.is_pinterest_url(url):
    ...
    def download(
        ...
        """
        Download a single Pinterest Pin (video or image).
        Tries yt-dlp first for videos; falls back to gallery-dl.
        """
        url = self.normalize_url(url)

        # Duplicate check
        if not skip_duplicate_check:
            is_dup, dup_date = self.history.is_downloaded(url, "pinterest")
            if is_dup:
                return False, f"Bu pin zaten indirilmiş! ({dup_date})"

        ydl_opts = self.get_base_options()
        ydl_opts.update({
            "outtmpl": str(self.pinterest_download_path / "%(title).200s [%(id)s].%(ext)s"),
        })

        if self.config.pinterest_cookies_file and os.path.exists(self.config.pinterest_cookies_file):
            ydl_opts["cookiefile"] = self.config.pinterest_cookies_file
        ...
        try:
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                ...
                if error_code == 0:
                    self.history.add_download(url, "pinterest")
                    return True, "Başarıyla indirildi (yt-dlp)"
                ytdlp_msg = f"yt-dlp hatası (kod: {error_code})"
        except Exception as e:
            ytdlp_msg = f"yt-dlp hatası: {e!s}"

        # Fallback to gallery-dl (handles pins, images, and special pin formats)
    ...
    def bulk_download(
        ...
        """
        Download multiple Pinterest pins concurrently.
        """
        if not urls:
            return 0, 0, 0, []
        ...
        for url in urls:
            if skip_duplicates:
                is_dup, _ = self.history.is_downloaded(url, "pinterest")
                if is_dup:
                    skipped += 1
    ...
    def bulk_download_from_file(
        ...
        """Download items from a text file containing Pinterest URLs."""
